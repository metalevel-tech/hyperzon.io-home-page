...
const isSafari = window.safari !== undefined; // Gallery accumulator class, used by the gallery handlers below
...
function galleryOverlayClear() {
  ...
  window.removeEventListener("orientationChange", calcHeightOnDeviceTilt); // document.removeEventListener("scroll", calcHeightOnDeviceTilt);
  // Clear images
  ...
  nodes.galleryButtonNext.removeEventListener("click", galleryChange_Image); // Clear additional classes from DOM
...
function galleryChange_Video(currentGallery, forward = true) {
  ...
  currentGallery.current = newVideo; // videoPlayer.pause();
...
function calcHeightOnDeviceTilt(node) {
  if (!node) return false; // Wait a while for styles to be applied
...
function galleryHandler_Images() {
  ...
  imageGalleriesOnAPage.forEach(function (gallery) {
    ...
    const currentGallery = new galleryUrlArray(false, imageItemsList.map(item => item.href)); // Close gallery Button functionality

    nodes.galleryButtonClose.addEventListener("click", galleryOverlayClear); // Process each image in the gallery
...
function galleryHandler_PostPageImgs() {
  ...
  const currentGallery = new galleryUrlArray(false, postPageAsImageGallery.map(item => item.src)); // Close gallery Button functionality
...
function galleryHandler_Videos() {
  ...
  videoGalleriesOnAPage.forEach(gallery => {
    ...
    const currentGallery = new galleryUrlArray(false, videoItemsList.map(item => item.querySelector("source").src)); // Close gallery Button functionality

    nodes.galleryButtonClose.addEventListener("click", galleryOverlayClear); // Process each video in the gallery
...
export { galleryHandler_Images, galleryHandler_PostPageImgs, galleryHandler_Videos };